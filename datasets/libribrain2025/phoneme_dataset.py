...
class LibriBrainPhoneme(LibriBrainBase):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            preload_event_file=preload_event_file,
            download=download,
        )
    ...
    def get_label(self, idx):
        """Get label for sample without loading MEG data."""
        if idx >= len(self.samples):
            raise IndexError(f"Index {idx} out of bounds")
        
        # Get label from samples directly
        sample = self.samples[idx]
        phoneme_full = sample[5]  # label is at index 5
