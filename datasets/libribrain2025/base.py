...
class LibriBrainBase(Dataset):
    ...
    def _open_h5_file(self, h5_path):
        """Open HDF5 file in standard read mode"""
    ...
    def __getitem__(self, idx):
        ...
        if self.standardize:
            # Use pre-computed channel means and stds directly
            data = (data - self.channel_means[:, np.newaxis]) / self.channel_stds[:, np.newaxis]
