...
class HDF5Dataset(Dataset):
    ...
    def __init__(
        ...
        use_memory_map: Enable memory-mapped file access for faster random reads (default: False)
        use_swmr: Enable Single Writer Multiple Reader mode for parallel access (default: False)
        h5_cache_size: Number of HDF5 file handles to cache (default: 100)
        """
        ...
        for subject in self.subjects:
            for session in self.sessions:
                for task in self.tasks:
                    h5_path = self._ids_to_h5_path(subject, session, task, self.runs[0])
                    h5_file = self._open_h5_file(h5_path)
                    hdf_dataset = h5_file["data"]
    ...
    def _get_sfreq(self):
        ...
        sfreq = info["sfreq"]
        h5_file.close()  # Close since we're not caching this
        return sfreq
    ...
    def _open_h5_file(self, h5_path):
        """Open HDF5 file with optimizations if enabled"""
        if self.use_memory_map:
            # Memory-mapped access for faster random reads
            return h5py.File(h5_path, 'r', driver='core', 
                           backing_store=False)  # Keep in memory
        elif self.use_swmr:
            # SWMR mode for parallel access
    ...
    def __getitem__(self, idx):
        ...
        if (not ((subject, session, task, run) in self.open_h5_datasets)):
            ...
            self._h5_access_order.append((subject, session, task, run))
            
            # Evict oldest if cache is full
            if len(self.open_h5_datasets) > self.h5_cache_size:
                oldest_key = self._h5_access_order.pop(0)
                del self.open_h5_datasets[oldest_key]
        else:
            h5_dataset = self.open_h5_datasets[(subject, session, task, run)]
            # Update access order for LRU
            if (subject, session, task, run) in self._h5_access_order:
                self._h5_access_order.remove((subject, session, task, run))
                self._h5_access_order.append((subject, session, task, run))
